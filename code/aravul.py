...
def get_workers_of_today(num_servers, p):
    ...
    for worker_ind in range(num_servers):
        if np.random.binomial(1, p, 1)[0]:
            ...
            class_mis.append(worker_ind)
    num_server_ = len(class_attendees)

    return class_attendees, class_mis, num_server_
...
def service_b(env, server_a, server_b, mu_a, mu_b, arrival_time, name,sigma_a, sigma_b, args, sim_num, num_caseta):
    ...
    with server_b.request() as req:
        ...
        server_b_df.loc[ind, 'time'] = env.now + sim_num * args.end_time
        server_b_df.loc[ind, 'daily_capacity'] = server_b.capacity
        server_b_df.loc[ind, 'day'] = sim_num
...
def service_a(env, server_a, server_b, mu_a, mu_b, arrival_time, name, sigma_a, sigma_b, args, sim_num, num_caseta):
    ...
    with server_a.request() as req:
        ...
        server_a_df.loc[ind, 'time'] = env.now + sim_num * args.end_time
        server_a_df.loc[ind, 'daily_capacity'] = server_a.capacity
        server_a_df.loc[ind, 'day'] = sim_num
...
def customer_arrivals(env, server_a, server_b, mu_a, mu_b, total_caseta, end_time, sigma_a, sigma_b, args,
                      sim_num, num_caseta, num_caseta_curr):
...
def main(args):
    ...
    throughput = []
    for num_caseta in tqdm([ 40, 41, 42 ,43,44, 45,46,47, 48]):
        ...
        number_of_ready_stage_0_df.loc[0,'num_ready'] = 10
        ...
        for sim_num in range(args.num_days):
            ...
            num_caseta_curr = num_caseta
            # who is attending to day
            # phase a
            num_server_a = 0
            while num_server_a == 0:
                class_a_attendees, class_mis_a, num_server_a = get_workers_of_today(args.num_servers_a, args.pa)

            # phase b
            num_server_b = 0
            while num_server_b == 0:
                class_b_attendees, class_mis_b, num_server_b = get_workers_of_today(args.num_servers_b, args.pb)

            with open('../pkl/server_surplus.pkl', 'rb') as f:
                surplus_df = pkl.load(f)

            ## Check if we need to decrease the number of casetas because someone did not show
            for ind in range(surplus_df.shape[0]):
                phase = surplus_df.loc[ind,'name'][0]
                if phase == 'b':
                    if (surplus_df.loc[ind,'surplus']>0) & (int(surplus_df.loc[ind,'name'][1:]) in class_mis_b):
                        num_caseta_curr += -1
            ...
            env.process(customer_arrivals(env, server_a, server_b,   args.mu_a, args.mu_b, num_caseta,  args.end_time,
                                          args.sigma_a, args.sigma_b, args, sim_num, num_caseta, num_caseta_curr))
...
def parse_arguments(argv):
    ...
    parser.add_argument('--mu_b', type=np.array, help='service rate', default= 1.25*np.array([3.73, 3.27, 2.86, 2.79, 2.74, 2.56,
    ...
    parser.add_argument('--num_days', type=int, help='total number of days in the simluation', default=300)
